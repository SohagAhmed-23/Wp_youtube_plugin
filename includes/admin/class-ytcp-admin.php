...
/**
 * Handles WordPress admin menus, settings registration, and manual sync actions.
 */
class YTCP_Admin {
	...
	/**
	 * Fields that are boolean toggles stored as '1' or '0'.
	 *
	 * @var array
	 */
	private $toggle_fields = array(
	...
	public function register_settings() {
		...
		$field_callbacks = array(
			'ytcp_api_key'              => array( $this, 'sanitize_api_key' ),
			'ytcp_channel_id'           => array( $this, 'sanitize_channel_id' ),
			...
			'ytcp_enable_transcripts'   => array( $this, 'sanitize_toggle' ),
			'ytcp_enable_history'       => array( $this, 'sanitize_toggle' ),
			'ytcp_enable_favorites'     => array( $this, 'sanitize_toggle' ),
			'ytcp_enable_autoplay'      => array( $this, 'sanitize_toggle' ),
			'ytcp_enable_pip'           => array( $this, 'sanitize_toggle' ),
			'ytcp_accent_color'         => 'sanitize_hex_color',
			'ytcp_sync_interval'        => array( $this, 'sanitize_sync_interval' ),
		);
	...
	/**
	 * Sanitizes the YouTube API key setting.
	 *
	...
	/**
	 * Sanitizes the YouTube Channel ID setting.
	 *
	 * @param string $value The raw input value.
	 * @return string
	 */
	...
	/**
	 * Sanitizes the sync interval setting to an allowed value.
	 *
	 * @param string $value The raw input value.
	 * @return string
	 */
	...
	/**
	 * Sanitizes a generic text field setting.
	 *
	 * @param string $value The raw input value.
	 * @return string
	 */
	...
	/**
	 * Sanitizes a boolean toggle setting ('1' or '0').
	 *
	...
	/**
	 * Runs an automatic first sync when settings are saved for the first time.
	 *
	 * @return void
	 */
	private function maybe_auto_sync() {
		...
		if ( ! empty( $sync_error ) && is_array( $sync_error ) ) {
			...
			add_action(
				'admin_notices',
				function () use ( $video_count, $playlist_count ) {
	...
	/**
	 * Flushes rewrite rules if the video or playlist slug has changed.
	 *
	 * @return void
	 */
	...
	/**
	 * Enqueues admin CSS and JS on plugin pages.
	 *
	 * @param string $hook The current admin page hook suffix.
	 * @return void
	 */
	public function enqueue_assets( $hook ) {
		if ( false === strpos( $hook, 'ytcp' ) ) {
			return;
		}

		wp_enqueue_media();
		wp_enqueue_style( 'ytcp-admin', YTCP_PLUGIN_URL . 'assets/css/admin.css', array(), YTCP_VERSION );
		wp_enqueue_script( 'ytcp-admin', YTCP_PLUGIN_URL . 'assets/js/admin.js', array( 'jquery' ), YTCP_VERSION, true );
		wp_localize_script(
			'ytcp-admin',
			'ytcpAdmin',
			array(
	...
	/**
	 * Renders the plugin dashboard admin page.
	 *
	 * @return void
	 */
	public function render_dashboard() {
		...
		<div class="wrap ytcp-admin">
			...
			<div class="ytcp-dashboard-grid">
				...
				<?php
				// Display API usage statistics for today.

				$api         = new YTCP_YouTube_API();
				$today       = gmdate( 'Y-m-d' );
				$stats       = $api->get_api_stats();
				$today_stats = $stats[ $today ] ?? array(
					'total_calls' => 0,
					'total_quota' => 0,
					'endpoints'   => array(),
				);
	...
	public function render_settings() {
							...
							<tr>
								...
								<td>
									...
									<div class="ytcp-image-preview" id="ytcp_hero_image_preview">
										<?php
										$img = get_option( 'ytcp_hero_image' );
										if ( $img ) :
											?>
							...
							<tr>
								...
								<td>
									...
									<div class="ytcp-image-preview" id="ytcp_channel_logo_preview">
										<?php
										$logo = get_option( 'ytcp_channel_logo' );
										if ( $logo ) :
											?>
		...
		<?php
		// End settings page output.
	}

	/**
	 * Renders the manual sync admin page.
	 *
	 * @return void
	 */
