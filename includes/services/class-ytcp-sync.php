...
/**
 * Handles cron-based and manual YouTube channel syncing.
 */
class YTCP_Sync {
	 ...
	 *
	 * @var YTCP_YouTube_API
	 */
	private $api;

	/**
	 * Constructor — instantiates the YouTube API service.
	 */
	...
	/**
	 * Registers a custom 'twice daily' cron schedule.
	 *
	...
	/**
	 * Schedules the sync cron event if not already scheduled.
	 *
	 * @return void
	 */
	...
	/**
	 * Runs the full channel sync — channel info and all playlists.
	 *
	 * @return void
	 */
	...
	/**
	 * Deletes all synced videos, playlists and related meta before a channel switch.
	 *
	 * @return void
	 */
	...
	/**
	 * Fetches and stores channel metadata (name, logo, banner, uploads playlist).
	 *
	 * @return void
	 */
	private function sync_channel_info() {
		...
		$branding        = $channel['brandingSettings'] ?? array();
		$content_details = $channel['contentDetails'] ?? array();
	...
	/**
	 * Fetches and syncs all playlists belonging to the configured YouTube channel.
	 *
	 * @return void
	 */
		...
		if ( ! empty( $uploads_id ) ) {
			...
			$uploads_data = array(
				...
				'snippet'        => array(
					...
					'description' => '',
					'publishedAt' => '',
					'thumbnails'  => array( 'high' => array( 'url' => '' ) ),
				),
				'contentDetails' => array( 'itemCount' => 0 ),
			);
	...
	/**
	 * Syncs all videos in a single YouTube playlist.
	 *
	 * @param string     $playlist_id   The YouTube playlist ID.
	 * @param array|null $playlist_data Optional pre-fetched playlist data.
	 * @return void
	 */
	...
	/**
	 * Creates or updates a ytcp_playlist post from YouTube API data.
	 *
	 * @param array $data The playlist data from the YouTube API.
	...
	/**
	 * Creates or updates a ytcp_video post from YouTube API playlist item data.
	 *
	 * @param array $item             Playlist item data from YouTube API.
	 * @param array $detail           Full video detail from the videos endpoint.
	 * @param int   $playlist_post_id The parent playlist post ID.
	...
	private function upsert_video( $item, $detail, $playlist_post_id, $position ) {
		...
		if ( $post_id && ! is_wp_error( $post_id ) ) {
			...
			if ( $detail ) {
				...
				$tags = $detail['snippet']['tags'] ?? array();
				if ( ! empty( $tags ) ) {
	...
	/**
	 * Triggers a full manual sync and increments the cache version.
	 *
	 * @return void
	 */
